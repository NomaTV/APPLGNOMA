...
function loginUser(PDO $db, string $username, string $password): void
{
    try {
        $stmt = $db->prepare("
            SELECT id_revendedor, usuario, senha, nome, master, ativo, limite_ativos
            FROM revendedores
            WHERE usuario = ?
        ");
        ...
        standardResponse(true, [
            'id_revendedor' => $user['id_revendedor'],
            'username' => $user['usuario'],
            'nome' => $user['nome'],
            'master' => $user['master'],
            'type' => determinarTipoUsuario($user['master']),
            'ativo' => (bool)$user['ativo'],
            'limite_ativos' => (int)$user['limite_ativos']
